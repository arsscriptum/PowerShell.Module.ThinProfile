...
try {
    ...
    $UpdateModuleVersionFileCmd = Get-Command "Update-ModuleVersionFile" -ErrorAction Ignore
    $SetThinProfileAutoUpdateOverrideCmd = Get-Command "Set-ThinProfileAutoUpdateOverride" -ErrorAction Ignore
    if(($UpdateModuleVersionFileCmd -eq $Null) -Or ($SetThinProfileAutoUpdateOverrideCmd -eq $Null)){
        [bool]$ModuleLoaded = $False
        Write-Host "MODULE NOT LOADED! NOT AUTO UPDATE nor PUBLISH" -ForegroundColor DarkRed
    }
...
"@
    ...
    if($ModuleLoaded){
        $ModuleVersion = Get-ThinProfileModuleVersion
        Set-ThinProfileAutoUpdateOverride -Enable $True
    }
    ...
    if($ModuleLoaded){
        Set-ThinProfileAutoUpdateOverride -Enable $False
    ...
    if(($ModuleLoaded) -And ($Deploy) ) {
        ...
        [version]$CurrVersionStruct = Get-ThinProfileModuleVersion
